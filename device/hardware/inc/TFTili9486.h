...
class LcdDriverDef {
  public:
    ...
    uint32_t Fill (ColorTypeDef __color, uint16_t __x = 480, uint16_t __y = 320);
    uint32_t Fill (const tImage &Image, uint32_t __scaleX = 1, uint32_t __scaleY = 1);
    uint32_t Fill (Frame2D<gbasic_t> &Frame);
	uint32_t Fill (Frame2D<gbasic_t> &Frame, b2D rect);
	//virtual uint32_t ExtInput (uint32_t , uint32_t , uint32_t);
	virtual int32_t ExtInput (b2D, uint32_t);
    uint16_t Init (uint32_t __mode);
  ...
  protected:
    ...
    void HsClk (void);
    void VsClk (void);
    void PutColor (ColorTypeDef __color);
	virtual void BcklApply (uint8_t);
}; 
union __IntToByte {
	uint8_t Byte[2];
	uint16_t Int;
